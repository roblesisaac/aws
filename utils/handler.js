...
global.getScriptsForSite = new Chain({
  ...
  steps: {
    forEachDefaultScriptsFromUiSheet: function() {
      ...
      models.sites.findOne({ name: "uisheet" }, function(err, uiSheet){
        if(err) return self.next(err);
        self.next(uiSheet.scripts);
      });
    },
    forEachDefaultScriptsFromUserSite: function() {
      this.next(this.site.scripts);
    },
    isViewingUserSite: function() {
      this.next(!!this.site);
    },
    appendToScripts: function (res) {
  ...
  instruct: [
    "forEachDefaultScriptsFromUiSheet", ["appendToScripts"],
    {
      if: "isViewingUserSite",
      true: [
        "forEachDefaultScriptsFromUserSite", ["appendToScripts"]
      ]
    }
  ]
});
...
global.serve = new Chain({
  ...
  steps: {
    ...
    convertSimpleTypeToMimeType: function(res) {
    	var type = this.types[res.type] || res.type || "application/json";
  	  ...
  	  false: [
  	    {
  	        ...
  	        { if: "thereAreVariables", true: "renderVariables" },
            {
      	      ...
      	      true: [ "assignCustomHeadersObj", "addBodyToFormatObj" ],
...
global.scripts = new Chain({
  ...
  steps: {
    masterSiteHasAltVersion: function() {
      ...
      models.sites.findOne({
        name: "uisheet"
          ...
          if(err) return self.error(err);
          self.masterScript = masterSite.scripts.findOne({
            name: self.sheetName
          });
    ...
    userSiteHasAltVersion: function() {
      var site = this.site;
      if(!site || !site.scripts) {
        this.next(false);
        return;
      }
      this.userSiteScript = this.site.scripts.findOne({
        name: this.sheetName
      });
