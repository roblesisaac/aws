    ...
    areThereAnyYet('users', firstUser, function(user) {
      ...
      areThereAnyYet('sites', firstSite, function(site) {
        const firstSheet = {
          ...
          js: "const login = (user) => { axios.post('https://www.blockometry.com/login', { username: user.username, password: user.password }).then(function(res) { window.sessionStorage['ply-token'] = res.data['ply-token']; window.sessionStorage.userid = res.data.userid; }); }; var instance = axios.create({ headers: { 'ply-token': window.sessionStorage['ply-token'], userid: window.sessionStorage.userid } }); var ply = new Vue({ created: function() { var vm = this; instance.get(this.ace.url).then(function(res) { vm.ace.send = res.data[0] || {}; vm.ace.url += vm.ace.send._id || ''; vm.ace.txt = vm.ace.send[vm.ace.prop] || '//enter code here'; }); }, data: { ace: { url: 'https://www.blockometry.com/plaza/api/sheets/', prop: 'js', send: null, txt: null }, link: sheets[0].name, sheets: sheets }, el: '#app', methods: { saveSheet: function() { this.ace.send[this.ace.prop] = this.ace.txt; instance.put(this.ace.url, this.ace.send).then(function(res) { console.log(res.data); }); } } });"
        };
